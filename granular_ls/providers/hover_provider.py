...
_STREAM_CONTEXT_DOCS = {
    ...
    'rng_group': (
        ...
        "di probabilita', distribuzione `iot`, selezione `window`, `detune` e "
        "voci stocastiche.\n\n"
        "Assente (default): identita' = `stream_id` — isolamento per-stream, "
        "comportamento invariato.\n\n"
        "Richiede `seed:` top-level per la riproducibilita' fra run. "
        "Limite: identita' condivisa non significa griglia temporale identica — "
    ...
    'duration_unit': (
        ...
        "  solo i valori Y).\n"
        "- `milliseconds`: i valori sono in **millisecondi**. Il fattore e' fisso\n"
        "  (`1e-3`) e, a differenza di `samples`, non dipende dalla frequenza di\n"
        "  output. E' la scala della grana udibile — fra 1 e 1000 ms — dove in\n"
        "  secondi si scrivono solo `.001` / `.0045` / `.35`.\n\n"
        "Con un'unita' **non-secondi** la `grain.duration` va indicata\n"
        "**esplicitamente**: il default `0.05` e' in secondi e non verrebbe\n"
        "convertito, quindi la base resterebbe in secondi mentre `duration_range`\n"
        "sarebbe nell'unita' dichiarata — due domini nello stesso blocco.\n\n"
        "La durata minima e' **1 campione** (`1/48000` s, ~20.8 µs) in ogni\n"
        "unita': `1` in campioni, `1/48` in millisecondi.\n\n"
        "> **Non e' un parametro sintetizzabile.** Non accetta envelope o range."
    ),
}
...
_BLOCK_KEY_DOCS = {
    ...
    'grain': (
        ...
        "> Con `duration_unit: samples` i valori sono in campioni (48000 Hz), con\n"
        "> `milliseconds` in millisecondi. In entrambe `duration` va indicata\n"
        "> esplicitamente (il default 0.05 è in secondi)."
    ),
    'deviation_probability': (
        "**deviation_probability** — Randomizzazione inter-grano\n\n"
        "Aggiunge dispersione casuale a parametri individuali tra i grani.\n\n"
        "**Valori accettati:**\n"
        "- `false` — nessuna probabilità: i range espliciti restano applicati\n"
        "  sempre (*range-only*), gli altri parametri non variano\n"
        "- numero — probabilità per tutti i parametri, in scala **0-100**\n"
        "  (`50` = metà dei grani; `true` vale `1`, cioè l'1%)\n"
        "- `envelope [[t, v], ...]` — la probabilità modulata nel tempo\n"
        "- `dict` — controllo per-parametro (chiave = nome parametro)\n\n"
        "> La scala è percentuale: `0.5` è mezzo punto percentuale, non metà\n"
        "> dei grani. Sotto `0` non si applica mai, da `100` in su sempre.\n\n"
        "**Esempio dict:**\n"
        "```yaml\n"
        "deviation_probability:\n"
        "  duration: 30\n"
        ...
        "**Cinque scritture si somigliano e una fa l'opposto:**\n\n"
        "| scrittura | esito |\n"
        "|---|---|\n"
        "| chiave assente | range-only |\n"
        "| `deviation_probability:` (vuota) | **jitter implicito 1%** |\n"
        "| `deviation_probability: {}` | range-only |\n"
        "| `deviation_probability: false` | range-only |\n"
        "| `<chiave>:` vuota dentro il dict | range-only |\n\n"
        "> **range-only** non vuol dire inerte: nessuna probabilità, ma il\n"
        "> range esplicito resta applicato **sempre**, a ogni grano. Con\n"
        "> `volume_range: 6` e `deviation_probability: false` la deviazione\n"
        "> c'è su tutti i grani; è senza range che non varia niente.\n\n"
        "> Scrivere la chiave e lasciarla vuota è la sola delle cinque che\n"
        "> **non** è range-only: accende una deviazione all'1% su ogni\n"
        "> parametro con un range — cioè la riduce, invece di lasciarla\n"
        "> piena. Per non averne, togli il range.\n\n"
        "> Un corpo che non si costruisce come envelope è un errore (PGE #209):\n"
        "> prima diventava probabilità 100% in silenzio, cioè l'opposto di\n"
        "> quanto un corpo malformato lascia supporre."
    ),
}
