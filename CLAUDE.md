...
| `loop_unit.py` | Registry di `pointer.loop_unit` (mirror di `LOOP_UNITS` / `_LOOP_UNIT_SCOPE` di PGE `PointerController`): vocabolario chiuso `seconds`/`absolute`/`normalized`, default `seconds` **indipendente da `time_mode`** (PGE #222), `find_loop_unit`, l'unico lettore della dichiarazione usato da hover, diagnostica e semantic token, e `stream_sample`, l'unico lettore del `sample` la cui durata limita le posizioni in secondi (nota dell'hover e bounds della diagnostica). Contiene anche il mirror di `_rescaling_would_change` per l'avviso di migrazione (ponytail, PGE #242). |
